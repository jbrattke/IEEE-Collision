...
int main() {
  ...
  string notes[12] = {"C5", "D5", "E5", "F5", "G5", "A5", "B5", "C6", "D6", "E6", "F6", "G6"} ;
  while (!exit) {
    ...
    if (userinput == 1)
    {
      for(int x=0; x<12; x++) 
      {
        sum=0; 
        string temp = notes[x];
        int counter = display->ReturnNote(temp); 
        display->RegisterWrite(MPCORE_PRIV_TIMER_LOAD_OFFSET, counter) ;
        display->RegisterWrite(MPCORE_PRIV_TIMER_CONTROL_OFFSET, 3) ;
      ...
      cin>>note;
      cout << "Enter the seconds that note will play: "<<endl;
      cin>>seconds;

      double cycles = seconds*200000000;

      int counter = display->ReturnNote(note);
