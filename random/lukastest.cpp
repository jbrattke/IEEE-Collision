...
class DE1SoCfpga 
{
  public:
    ...
    DE1SoCfpga() 
    {
     ...
     int video_resolution = RegisterRead(PIXEL_BUF_CTRL_BASE + 0x8)  ;
    screen_x = video_resolution & 0xFFFF;
    screen_y = (video_resolution >> 16) & 0xFFFF;

    int rgb_status = RegisterRead(RGB_RESAMPLER_BASE - LW_BRIDGE_BASE);
  ...
  {
    * (volatile unsigned int *)(pBase_Pixel + (y<<10) + (x<<1)) = colour ;
  }
...
void video_box(int x1, int y1, int x2, int y2, short int pixel_color) 
  { 
    ...
    for (row = y1; row <= y2; row++)
      ...
      {
        * (volatile short int *)(pBase_Pixel + (row << (10 - res_offset - col_offset)) + (col << 1)) = pixel_color ;
      }
  }
...
int main(void) 
{
  ...
  main->video_box(31 * 4, 28 * 4, 49 * 4 - 1, 32 * 4 - 1, INTEL_BLUE);
  temp++;
  }
}
